# Modified from https://github.com/BorealisAI/flora-opt/blob/main/flora_opt/optimizers/torch/flora.py
...
) -> Callable[[Strategy, Optimizer, dict, torch.Tensor, Optional[torch.Tensor]], None]:
    def wrapper(strategy, optimizer, group, p, *args, **kwargs):
        with torch.no_grad():
            # Compress and clear grad
            if hasattr(strategy, "is_factorized") and strategy.is_factorized(p):
                seed = strategy.seeds[p]
                strategy.packs[p] = strategy._down_project(
                    seed, p.grad
                ) + strategy.packs.get(p, 0.0)
            else:
                if p in strategy.packs:
                    strategy.packs[p].add_(p.grad)
                else:
                    strategy.packs[p] = p.grad

            p.grad = None

            if not strategy.should_accumulate():
                # Decompress and update grad
                if hasattr(strategy, "is_factorized") and strategy.is_factorized(p):
                    p.grad = strategy._up_project(
                        strategy.seeds[p], p, strategy.packs[p]
                    )
                    strategy.seeds[p] = next_seed(seed)
                else:
                    p.grad = strategy.packs[p]

                del strategy.packs[p]
...
class FloraFabricStrategy:
    """Mixin class to add Flora-specific functionality to any Fabric strategy"""

    def __init__(self, accumulation_compression_rank: Optional[int] = None):
        self.packs = {}
        self.seeds = {}
        self.proj_rank = accumulation_compression_rank
        self._accumulation_compression = accumulation_compression_rank is not None
        ...
        for group in optimizer.param_groups:
            for p in group["params"]:
                ...
                p.register_post_accumulate_grad_hook(
                    partial(with_flora_fabric(flora_step_), self, optimizer, group)
                )
        return optimizer


class FloraDDPStrategy(FloraFabricStrategy, DDPStrategy):
    """Flora optimizer strategy integrated with DDP"""

    def __init__(
        self, *args, accumulation_compression_rank: Optional[int] = None, **kwargs
    ):
        FloraFabricStrategy.__init__(self, accumulation_compression_rank)
        DDPStrategy.__init__(self, *args, **kwargs)

    def should_accumulate(self) -> bool:
        """Check if we should continue to accumulate gradients"""
        if hasattr(self, "_optimizer") and hasattr(self._optimizer, "grad_accum_steps"):
            return self._optimizer.grad_accum_count < self._optimizer.grad_accum_steps
        return False


class FloraFSDPStrategy(FloraFabricStrategy, FSDPStrategy):
    """Flora optimizer strategy integrated with FSDP"""

    def __init__(
        self,
        *args,
        accumulation_compression_rank: Optional[int] = None,
        ...
        **kwargs
        ...
        self.cpu_offload = CPUOffload() if cpu_offload else None
        self.mixed_precision = mixed_precision
        self.sharding_strategy = sharding_strategy
        self.backward_prefetch = backward_prefetch

        FSDPStrategy.__init__(
            self,
            *args,
    ...
    def _setup_model(self, model):
        """Additional setup for FSDP wrapped models"""
        model = super()._setup_model(model)
        # Ensure Flora's hooks are properly registered after FSDP wrapping
        if isinstance(model, FSDP):
            self._register_flora_hooks(model)
        return model

    def _register_flora_hooks(self, model):
        """Register Flora hooks on FSDP wrapped parameters"""
        for param in model.parameters():
            if param.requires_grad:
                if self.is_factorized(param):
                    self.seeds[param] = len(self.seeds)
                param.register_post_accumulate_grad_hook(
                    partial(
                        with_flora_fabric(flora_step_),
                        self,
                        self._optimizer,
                        self._optimizer.param_groups[0],
                    )
                )


class FloraSingleDeviceStrategy(FloraFabricStrategy, SingleDeviceStrategy):
    """Flora optimizer strategy for single device training"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        FloraFabricStrategy.__init__(self, accumulation_compression_rank)
        SingleDeviceStrategy.__init__(self, device=device, *args, **kwargs)
    ...
    def _setup_model(self, model):
        """Setup for single device model"""
        model = super()._setup_model(model)
        self._register_flora_hooks(model)
        return model

    def _register_flora_hooks(self, model):
        """Register Flora hooks on model parameters"""
        for param in model.parameters():
            if param.requires_grad:
                if self.is_factorized(param):
                    self.seeds[param] = len(self.seeds)
...
# Register the following strategies
StrategyRegistry.register(
    "ddp_flora",
    FloraDDPStrategy,
    description="Flora optimizer strategy integrated with DDP",
)
StrategyRegistry.register(
    "fsdp_flora",
    FloraFSDPStrategy,
    description="Flora optimizer strategy integrated with FSDP",
)
StrategyRegistry.register(
    "single_flora",
    FloraSingleDeviceStrategy,
    description="Flora optimizer strategy for single device training",
)
