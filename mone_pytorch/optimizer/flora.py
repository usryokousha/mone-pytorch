...
def with_flora_fabric(
    ...
    def wrapper(fabric, optimizer, group, p, *args, **kwargs):
        with torch.no_grad():
            ...
            if not fabric.is_factorized(p):
                if p in fabric.packs:
                    fabric.packs[p].add_(p.grad)
                else:
            ...
            if not fabric.sync_gradients:
                return

            # Decompress and update grad
            if not fabric.is_factorized(p):
                p.grad = fabric.packs[p]
            else:
                p.grad = fabric._up_project(fabric.seeds[p], p, fabric.packs[p])
                fabric.seeds[p] = next_seed(seed)
...
class FloraStrategyMixin:
    def __init__(self, accumulation_rank: int = None):
        self.accumulation_rank = accumulation_rank
        self.packs = {}
        self.seeds = {}
        self.proj_rank = accumulation_rank

    def is_factorized(self, p: torch.Tensor) -> bool:
        return p.dim() >= 2 and self._accumulation_rank is not None
    ...
    for group in optimizer.param_groups:
        for p in group["params"]:
            if not p.requires_grad:
                continue
            if strategy.is_factorized(p):
                strategy.seeds[p] = len(strategy.seeds)
